...
def assessment_content_check(request):
    ...
    receiver=["amit@sutara.org","nikhilchoudhary@sutara.org","bburrage@curiouslearning.org","ashish.m@sutara.org"]
    ...
    assesment_data=get_assessment_data(gc,sheet_id,tab_number)
...
def get_assessment_data(gc, sheet_id,tab_number):
    # connect to the spreadsheet
    fetched_sheet = gc.open_by_key(sheet_id)
    fetched_content = fetched_sheet[int(tab_number)-1].get_values("B1","B150")
...
def get_correct_file_name(file_name):
    ...
    return correct_file_name.lower()
